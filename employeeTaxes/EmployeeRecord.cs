    ...
    public class EmployeeRecord : Global {
        ...
        public EmployeeRecord( string csvLine, int lineErr ) {
            ...
            if (int.TryParse(lineParse[0], out int id)) Id = id;
                ...
                BreakError<string>(15, $"Employee id format invalid. Expected a non-decimal number, received '{lineParse[0]}'.", LineError, FileName);

            // parse name
            if (!new Regex("^[a-zA-Z]+$").IsMatch(lineParse[1]))
                BreakError<string>(15, $"Employee name can only have alpha characters, received '{lineParse[1]}'.", LineError, FileName);
            else Name = lineParse[1];

            // parse state
            if (!new Regex("^[a-zA-Z]+$").IsMatch(lineParse[2]) && lineParse[2].Length != 2)
                BreakError<string>(15, $"State can only be 2 alpha characters, received '{lineParse[2]}'.", LineError, FileName);
            else if (!TaxCalculator.record.ContainsKey(lineParse[2]))
                BreakError<string>(15, $"Employee's State is not available in the current registery, received '{lineParse[2]}'", LineError, FileName);
            else State = lineParse[2];

            // parse hours
            if (int.TryParse(lineParse[3], out int hour)) Hours = hour;
            else 
                BreakError<string>(15, $"Employee Hours format invalid. Expected a non-decimal number, received '{lineParse[3]}'.", LineError, FileName);

            // parse pay
            if (decimal.TryParse(lineParse[4], out decimal pay)) Pay = pay;
            else 
                BreakError<string>(15, $"Employee Hourly Pay format invalid. Expected a decimal number, received '{lineParse[4]}'.", LineError, FileName);
        }
        public int TotalPay => (int)(Hours * Pay);
        public decimal TaxDue => State != null && TotalPay > 0
            ? TaxCalculator.ComputeTaxFor(State, TotalPay)
            : BreakError<decimal>(15, "There was an error calculating tax.", LineError, FileName);
